...
connection = pymysql.connect(host='localhost',user='root',password='',db='djangobackend')
cursor = connection.cursor()


# inserting miscellaneous data, fetched results, prediction and recommendation into user_data table
def insert_data(sec_token,ip_add,host_name,dev_user,os_name_ver,latlong,city,state,country,act_name,act_mail,act_mob,name,email,res_score,timestamp,no_of_pages,reco_field,cand_level,skills,recommended_skills,courses,pdf_name):
    DB_table_name = 'candidates_resume'
    insert_sql = "insert into " + DB_table_name + "(sec_token,ip_add,host_name,dev_user,os_name_ver,latlong,city,state,country,act_name,act_mail,act_mob,name,email,res_score,timestamp,no_of_pages,reco_field,cand_level,skills,recommended_skills,courses,pdf_name)" + """
    values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
    rec_values = (str(sec_token),str(ip_add),host_name,dev_user,os_name_ver,str(latlong),city,state,country,act_name,act_mail,act_mob,name,email,str(res_score),timestamp,str(no_of_pages),reco_field,cand_level,skills,recommended_skills,courses,pdf_name)
    cursor.execute(insert_sql, rec_values)
    
    resume_id_query = "select Id from candidates_resume where email=" + """%s""" 
    #values = (email,sec_token)
    cursor.execute(resume_id_query,email)
    cv_id_tuple = cursor.fetchone()
    cv_id = cv_id_tuple[0]
    print(cv_id)
    add_resume_to_candidate = "UPDATE candidates_candidate SET cv_id =" + '%s' + "WHERE (id =" + """%s)"""
    n_values = (str(cv_id),str(candidate_id))
    cursor.execute(add_resume_to_candidate, n_values)
    
    connection.commit()
    #UPDATE `djangobackend`.`candidates_candidate` SET `cv_id` = '4' WHERE (`id` = '9');
...
# Retrieve the candidateId
candidate_id = params.get('candidateId', [None])[0]  # Default to None if not found

# Display the candidateId
if candidate_id:
    st.write(f"Candidate ID: {candidate_id}")
else:
    st.write("No candidate ID provided.")
###### Main function run() ######
...
def run():
    ...
    if choice == 'User':
        ...
        # Upload Resume
        st.markdown('''<h5 style='text-align: left; color: #00203a;'> Upload Your Resume, And Get Smart Recommendations</h5>''',unsafe_allow_html=True)
